...
class Seq2SeqSumm(nn.Module):
    def __init__(self, vocab_size, emb_dim,
        ...
        #project coverage into feature
        self.c_f = nn.Linear(1, n_hidden, bias=False)
        self._coverage = nn.Linear(2*n_hidden, n_hidden, bias=False)
...
class AttentionalLSTMDecoder(object):
    def __init__(self, embedding, lstm, attn_w, projection, coverage_projection, coverage_feature):
        super().__init__()
        self._embedding = embedding
        self._lstm = lstm
        self._attn_w = attn_w
        self._projection = projection
        self._coverage = coverage_projection
        self.c_f = coverage_feature

    def __call__(self, attention, init_states, target):
        batch_size = target.size(0)
        max_len = target.size(1)
        s_len = attention[1].size(1)
        states = init_states
        logits = []
        coverage = torch.zeros(batch_size, s_len).to(target.device)
        score_list = []
        coverage_list = []
        for i in range(max_len):
            tok = target[:, i:i+1]
            logit, states, score, coverage = self._step(tok, states, attention, coverage)
            logits.append(logit)
            score_list.append(score)
            coverage_list.append(coverage)
        logit = torch.stack(logits, dim=1)
        score = torch.stack(score_list, dim=1)
        coverage = torch.stack(coverage_list, dim=1)
        return logit, score, coverage
    ...
    def decode_step(self, tok, states, attention, coverage):
        logit, states, score, coverage = self._step(tok, states, attention, coverage)
