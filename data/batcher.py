...
def batchify_fn_copy(pad, start, end, data, cuda=True):
    ...
    src_lens = [len(src) for src in sources]
    abs_lens = [len(src) for src in tar_ins]
    sources = [src for src in sources]
    ext_srcs = [ext for ext in ext_srcs]
    ext_abses = [abs for abs in targets]
    inp_abses = [inp for inp in tar_ins]
    inp_abs = pad_batch_tensorize(inp_abses, pad, cuda)
    ext_abs = pad_batch_tensorize(ext_abses, pad, cuda)
    ...
    vsize = source.max().item() + 1
    fw_args = (source, src_lens, tar_in, ext_src, ext_vsize, vsize, abs_lens, inp_abs, ext_abs)
    loss_args = (target, )
    return fw_args, loss_args
